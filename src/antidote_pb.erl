...
%% enumerated types
-type 'CRDT_type'() :: 'COUNTER' | 'ORSET' | 'LWWREG' | 'MVREG' | 'GMAP' | 'RWSET' | 'RRMAP' | 'FATCOUNTER' | 'FLAG_EW' | 'FLAG_DW' | 'GENERIC'.
...
d_field_ApbCounterUpdate_inc(<<0:1, X:7, Rest/binary>>,
    ...
    {NewFValue, RestF} = {begin
			    ZValue = X bsl N + Acc,
			    if ZValue band 1 =:= 0 ->
				   id(ZValue bsr 1, {var, 970, TrUserData});
			       true ->
				   id(-(ZValue + 1 bsr 1),
				      {var, 970, TrUserData})
			    end
...
enum_symbol_by_value_CRDT_type(15) -> 'GENERIC'.
...
enum_value_by_symbol_CRDT_type('GENERIC') -> 15.
