...
-export([decode_request/1, decode_response/1,
	 encode_request/1, encode_response/1]).

% these are all top-level messages which can be sent on the wire
-export_type([request/0, response/0, update/0,
	      read_result/0]).

-type bound_object() :: {Key :: binary(),
			 Type :: atom(), Bucket :: binary()}.

-type update() :: {Object :: bound_object(),
		   Op :: atom(), Param :: any()}.

-type error_code() :: unknown | timeout |
		      no_permissions | aborted | {error_code, integer()}.

-type read_result() :: {counter, integer()} |
		       {set, [binary()]} | {reg, binary()} |
		       {mvreg, [binary()]} |
		       {map,
			[{{Key :: binary(), Type :: atom()},
			  Value :: read_result()}]} |
		       {generic, binary()} | {flag, boolean()}.

-type read_result_in() :: {antidote_crdt_counter_fat,
			   integer()} |
			  ...
			  {antidote_crdt_map_go,
			   [{{Key :: binary(), Type :: atom()},
			     Value :: read_result_in()}]} |
			  {antidote_crdt_map_rr,
			   [{{Key :: binary(), Type :: atom()},
			     Value :: read_result_in()}]} |
			  {antidote_crdt_generic, binary()} |
			  {antidote_crdt_flag_dw, boolean()} |
			  {antidote_crdt_flag_ew, boolean()}.

-type request() :: {start_transaction,
		    Clock :: binary(), Properties :: list()} |
		   {abort_transaction, TxId :: binary()} |
		   {commit_transaction, TxId :: binary()} |
		   {update_objects, Updates :: [update()],
		    TxId :: binary()} |
		   {static_update_objects, Clock :: binary(),
		    Properties :: list(), Updates :: [update()]} |
		   {static_read_objects, Clock :: binary(),
		    Properties :: list(), Objects :: [bound_object()]} |
		   {read_objects, Objects :: [bound_object()],
		    TxId :: binary()} |
...
-type response() :: {error_response,
		     {ErrorCode :: error_code(), Message :: binary()}} |
		    {start_transaction_response,
		     {ok, TxId :: binary()} |
		     {error, Reason :: error_code()}} |
		    {commit_transaction_response,
		     {ok, CommitTime :: binary()} |
		     {error, Reason :: error_code()}} |
		    {static_read_objects_response,
		     {ok, Results :: [read_result()],
		      CommitTime :: binary()}} |
		    {read_objects_response,
		     {ok, Resp :: [read_result()]} |
		     {error, Reason :: error_code()}} |
		    {operation_response,
		     ok | {error, Reason :: error_code()}} |
		    {create_dc_response,
		     ok | {error, Reason :: error_code()}} |
		    {get_connection_descriptor_response,
		     {ok, Descriptor :: binary()} |
		     {error, Reason :: error_code()}} |
		    {connect_to_dcs_response,
		     ok | {error, Reason :: error_code()}}.

-type sendable() :: #'ApbErrorResp'{} |
		    #'ApbStartTransaction'{} |
		    #'ApbStartTransactionResp'{} |
		    #'ApbAbortTransaction'{} | #'ApbCommitTransaction'{} |
		    #'ApbCommitResp'{} | #'ApbUpdateObjects'{} |
		    #'ApbStaticUpdateObjects'{} |
		    #'ApbStaticReadObjects'{} |
		    #'ApbStaticReadObjectsResp'{} | #'ApbReadObjects'{} |
		    #'ApbReadObjectsResp'{} | #'ApbOperationResp'{} |
		    #'ApbCreateDC'{} | #'ApbCreateDCResp'{} |
		    #'ApbGetConnectionDescriptor'{} |
		    #'ApbGetConnectionDescriptorResp'{} |
		    #'ApbConnectToDCs'{} | #'ApbConnectToDCsResp'{}.

-define(ASSERT_BINARY(X),
	case is_binary(X) of
	  true -> ok;
	  false -> throw({not_binary, X})
	end).

-define(ASSERT_ALL_BINARY(Xs),
	[?ASSERT_BINARY(X) || X <- Xs]).
...
encode_request(Data) -> encode(Data).
...
encode_response(Data) ->
    TransformReadResponse = case Data of
			      {static_read_objects_response,
			       {Results, CommitTime}} ->
				  TransformedResults = [{encode_crdt_type(Type),
							 Value}
							|| {{_Key, Type,
							     _Bucket},
							    Value}
...
message_type_to_code('ApbGetConnectionDescriptor') ->
    133;
message_type_to_code('ApbGetConnectionDescriptorResp') ->
    134.
...
message_code_to_type(130) -> 'ApbCreateDCResp';
message_code_to_type(131) -> 'ApbConnectToDCs';
message_code_to_type(132) -> 'ApbConnectToDCsResp';
message_code_to_type(133) ->
    'ApbGetConnectionDescriptor';
message_code_to_type(134) ->
    'ApbGetConnectionDescriptorResp'.

-spec encode(message()) -> iolist().

encode(Msg) -> encode_msg(encode_message(Msg)).
...
encode_msg(Msg) ->
    ...
    [message_type_to_code(MsgType),
     [antidote_pb:encode_msg(Msg)]].
...
encode_message({start_transaction, Clock,
		Properties}) ->
...
encode_message({static_update_objects, Clock,
		Properties, Updates}) ->
    encode_static_update_objects(Clock, Properties,
				 Updates);
encode_message({static_read_objects, Clock, Properties,
		Objects}) ->
...
encode_message({commit_transaction_response, Resp}) ->
...
encode_message({get_connection_descriptor_response,
		Resp}) ->
    encode_get_connection_descriptor_response(Resp);
encode_message({create_dc, Nodes}) ->
    encode_create_dc(Nodes);
encode_message({create_dc_response, Resp}) ->
    encode_create_dc_response(Resp);
encode_message({connect_to_dcs, Descriptors}) ->
    encode_connect_to_dcs(Descriptors);
encode_message({connect_to_dcs_response,
		Descriptors}) ->
    encode_connect_to_dcs_response(Descriptors).

-spec decode_message(sendable()) -> message().

decode_message(#'ApbStartTransaction'{properties =
					  Properties,
				      timestamp = Clock}) ->
    {start_transaction, Clock,
     decode_txn_properties(Properties)};
decode_message(#'ApbAbortTransaction'{transaction_descriptor
					  = TxId}) ->
    {abort_transaction, TxId};
decode_message(#'ApbCommitTransaction'{transaction_descriptor
					   = TxId}) ->
    {commit_transaction, TxId};
decode_message(#'ApbUpdateObjects'{updates = Updates,
				   transaction_descriptor = TxId}) ->
    {update_objects, [decode_update_op(U) || U <- Updates],
     TxId};
decode_message(#'ApbStaticUpdateObjects'{updates =
					     Updates,
					 transaction = Tx}) ->
    Clock = Tx#'ApbStartTransaction'.timestamp,
    Properties =
	decode_txn_properties(Tx#'ApbStartTransaction'.properties),
    {static_update_objects, Clock, Properties,
     [decode_update_op(U) || U <- Updates]};
decode_message(#'ApbStaticReadObjects'{objects =
					   Objects,
				       transaction = Tx}) ->
    Clock = Tx#'ApbStartTransaction'.timestamp,
    Properties =
	decode_txn_properties(Tx#'ApbStartTransaction'.properties),
    {static_read_objects, Clock, Properties,
     [decode_bound_object(O) || O <- Objects]};
decode_message(#'ApbReadObjects'{boundobjects = Objects,
				 transaction_descriptor = TxId}) ->
    {read_objects, [decode_bound_object(O) || O <- Objects],
     TxId};
decode_message(#'ApbCreateDC'{nodes = Nodes}) ->
    {create_dc, [binary_to_atom(N, utf8) || N <- Nodes]};
decode_message(#'ApbCreateDCResp'{success = false,
				  errorcode = E}) ->
    {create_dc_response, {error, decode_error_code(E)}};
decode_message(#'ApbCreateDCResp'{success = true,
				  errorcode = _E}) ->
    {create_dc_response, ok};
decode_message(#'ApbGetConnectionDescriptor'{}) ->
    get_connection_descriptor;
decode_message(#'ApbGetConnectionDescriptorResp'{success
						     = false,
						 descriptor = _D,
						 errorcode = E}) ->
    {get_connection_descriptor_response,
...
decode_message(#'ApbGetConnectionDescriptorResp'{success
						 ...
						 errorcode = _E}) ->
    {get_connection_descriptor_response, {ok, Descriptor}};
decode_message(#'ApbConnectToDCs'{descriptors =
				      Descriptors}) ->
    {connect_to_dcs, Descriptors};
decode_message(#'ApbConnectToDCsResp'{success = false,
				      errorcode = E}) ->
    {connect_to_dcs_response,
     {error, decode_error_code(E)}};
decode_message(#'ApbConnectToDCsResp'{success = true,
				      errorcode = _E}) ->
    {connect_to_dcs_response, ok};
decode_message(#'ApbErrorResp'{errcode = ErrorCode,
			       errmsg = Message}) ->
    {error_response,
     {decode_error_code(ErrorCode), Message}};
decode_message(#'ApbStartTransactionResp'{success =
					      Success,
					  transaction_descriptor = TxId,
					  errorcode = ErrorCode}) ->
...
decode_message(#'ApbCommitResp'{success = Success,
				errorcode = ErrorCode, commit_time = Time}) ->
    Resp = case Success of
	     true -> {ok, Time};
	     false -> {error, decode_error_code(ErrorCode)}
	   end,
    {commit_transaction_response, Resp};
decode_message(#'ApbStaticReadObjectsResp'{objects =
					       #'ApbReadObjectsResp'{objects =
									 Objects},
					   committime =
					       #'ApbCommitResp'{commit_time =
								    Time}}) ->
    Results = [decode_read_object_resp(O) || O <- Objects],
    {static_read_objects_response, {Results, Time}};
decode_message(#'ApbReadObjectsResp'{success = Success,
				     errorcode = ErrorCode,
				     objects = Objects}) ->
    case Success of
      true ->
...
decode_message(#'ApbOperationResp'{success = S,
				   errorcode = E}) ->
    case S of
      true -> {operation_response, ok};
      false ->
...
encode_error_resp(ErrorCode, Message) ->
    #'ApbErrorResp'{errcode = encode_error_code(ErrorCode),
		    errmsg = Message}.
...
encode_txn_properties(Props) ->
    #'ApbTxnProperties'{shared_locks =
			    proplists:get_value(shared_locks, Props, []),
			exclusive_locks =
			    proplists:get_value(exclusive_locks, Props, [])}.

decode_txn_properties(#'ApbTxnProperties'{shared_locks =
					      SharedLocks,
					  exclusive_locks = ExclusiveLocks}) ->
    [{shared_locks, all_to_binary(SharedLocks)}
     || SharedLocks /= []]
      ++
      [{exclusive_locks, all_to_binary(ExclusiveLocks)}
       || ExclusiveLocks /= []].

-spec all_to_binary([iodata()]) -> [binary()].

all_to_binary(L) -> [iodata_to_binary(B) || B <- L].
...
encode_bound_object(Key, Type, Bucket) ->
    #'ApbBoundObject'{key = Key, type = encode_type(Type),
		      bucket = Bucket}.

decode_bound_object(Obj) ->
    #'ApbBoundObject'{key = Key, type = Type,
		      bucket = Bucket} =
	Obj,
...
encode_static_update_objects(Clock, Properties,
			     Updates) ->
    EncTransaction = encode_start_transaction(Clock,
					      Properties),
    EncUpdates = lists:map(fun (Update) ->
				   encode_update_op(Update)
			   end,
			   Updates),
...
decode_update_op(Obj) ->
    #'ApbUpdateOp'{boundobject = Object,
		   operation = Operation} =
	Obj,
...
encode_update_op(Object, Op, Param) ->
    ...
    #'ApbUpdateOp'{boundobject = EncObject,
		   operation = Operation}.
...
encode_update_objects(Updates, TxId) ->
    EncUpdates = lists:map(fun (Update) ->
			   ...
			   Updates),
    #'ApbUpdateObjects'{updates = EncUpdates,
			transaction_descriptor = TxId}.
...
encode_start_transaction_response({ok, TxId}) ->
    ?ASSERT_BINARY(TxId),
    #'ApbStartTransactionResp'{success = true,
			       transaction_descriptor = TxId}.
...
encode_commit_response({ok, CommitTime}) ->
    ...
    #'ApbCommitResp'{success = true,
		     commit_time = CommitTime}.

%%%%%%%%%%%%%%%%%%%%%%
%% Reading objects

encode_static_read_objects_response({Results,
				     CommitTime}) ->
    ?ASSERT_BINARY(CommitTime),
...
encode_read_objects_response({ok, Results}) ->
    #'ApbReadObjectsResp'{success = true,
			  objects =
			      [encode_read_object_resp(Type, Value)
			       || {Type, Value} <- Results]};
encode_read_objects_response({error, Reason}) ->
    #'ApbReadObjectsResp'{success = false,
			  errorcode = encode_error_code(Reason)}.

encode_static_read_objects(Clock, Properties,
			   Objects) ->
    ?ASSERT_BINARY(Clock),
    EncTransaction = encode_start_transaction(Clock,
					      Properties),
    EncObjects = lists:map(fun (Object) ->
				   encode_bound_object(Object)
			   end,
			   Objects),
...
encode_read_objects(Objects, TxId) ->
    BoundObjects = lists:map(fun (Object) ->
			     ...
			     Objects),
    #'ApbReadObjects'{boundobjects = BoundObjects,
		      transaction_descriptor = TxId}.
...
decode_type(T) ->
    erlang:error({unknown_crdt_type_protobuf, T}).
...
encode_update_operation(antidote_crdt_counter_pn,
			Op_Param) ->
    #'ApbUpdateOperation'{counterop =
			      encode_counter_update(Op_Param)};
encode_update_operation(antidote_crdt_counter_fat,
			Op_Param) ->
    #'ApbUpdateOperation'{counterop =
			      encode_counter_update(Op_Param)};
encode_update_operation(antidote_crdt_counter_b,
			Op_Param) ->
    #'ApbUpdateOperation'{counterop =
			      encode_counter_update(Op_Param)};
encode_update_operation(antidote_crdt_set_aw,
			Op_Param) ->
    #'ApbUpdateOperation'{setop =
			      encode_set_update(Op_Param)};
encode_update_operation(antidote_crdt_set_rw,
			Op_Param) ->
    #'ApbUpdateOperation'{setop =
			      encode_set_update(Op_Param)};
encode_update_operation(antidote_crdt_register_lww,
			Op_Param) ->
    #'ApbUpdateOperation'{regop =
			      encode_reg_update(Op_Param)};
encode_update_operation(antidote_crdt_register_mv,
			Op_Param) ->
    #'ApbUpdateOperation'{regop =
			      encode_reg_update(Op_Param)};
encode_update_operation(antidote_crdt_map_go,
			Op_Param) ->
    #'ApbUpdateOperation'{mapop =
			      encode_map_update(Op_Param)};
encode_update_operation(antidote_crdt_map_rr,
			Op_Param) ->
    #'ApbUpdateOperation'{mapop =
			      encode_map_update(Op_Param)};
encode_update_operation(antidote_crdt_flag_ew,
			Op_Param) ->
    #'ApbUpdateOperation'{flagop =
			      encode_flag_update(Op_Param)};
encode_update_operation(antidote_crdt_flag_dw,
			Op_Param) ->
    #'ApbUpdateOperation'{flagop =
			      encode_flag_update(Op_Param)};
encode_update_operation(antidote_crdt_generic,
			Op_Param) ->
    #'ApbUpdateOperation'{genop =
			      encode_generic_update(Op_Param)};
encode_update_operation(Type, _Op) ->
    throw({invalid_type, Type}).

decode_update_operation(#'ApbUpdateOperation'{counterop
						  = Op})
    when Op /= undefined ->
    decode_counter_update(Op);
decode_update_operation(#'ApbUpdateOperation'{setop =
						  Op})
    when Op /= undefined ->
    decode_set_update(Op);
decode_update_operation(#'ApbUpdateOperation'{regop =
						  Op})
    when Op /= undefined ->
    decode_reg_update(Op);
decode_update_operation(#'ApbUpdateOperation'{mapop =
						  Op})
    when Op /= undefined ->
    decode_map_update(Op);
decode_update_operation(#'ApbUpdateOperation'{flagop =
						  Op})
    when Op /= undefined ->
    decode_flag_update(Op);
decode_update_operation(#'ApbUpdateOperation'{genop =
						  Op})
    when Op /= undefined ->
    decode_generic_update(Op);
decode_update_operation(#'ApbUpdateOperation'{resetop =
						  #'ApbCrdtReset'{}}) ->
    {reset, {}}.

encode_crdt_type(antidote_crdt_register_lww) -> reg;
encode_crdt_type(antidote_crdt_register_mv) -> mvreg;
encode_crdt_type(antidote_crdt_counter_pn) -> counter;
encode_crdt_type(antidote_crdt_counter_fat) -> counter;
encode_crdt_type(antidote_crdt_set_aw) -> set;
encode_crdt_type(antidote_crdt_set_rw) -> set;
encode_crdt_type(antidote_crdt_map_go) -> map;
encode_crdt_type(antidote_crdt_map_rr) -> map;
encode_crdt_type(antidote_crdt_flag_ew) -> flag;
encode_crdt_type(antidote_crdt_flag_dw) -> flag;
encode_crdt_type(antidote_crdt_generic) -> generic.

encode_read_object_resp(reg, Val) ->
    #'ApbReadObjectResp'{reg =
			     #'ApbGetRegResp'{value = Val}};
encode_read_object_resp(mvreg, Val) ->
    #'ApbReadObjectResp'{mvreg =
			     #'ApbGetMVRegResp'{values = Val}};
encode_read_object_resp(counter, Val) ->
    #'ApbReadObjectResp'{counter =
			     #'ApbGetCounterResp'{value = Val}};
encode_read_object_resp(set, Val) ->
    #'ApbReadObjectResp'{set =
			     #'ApbGetSetResp'{value = Val}};
encode_read_object_resp(map, Val) ->
    #'ApbReadObjectResp'{map = encode_map_get_resp(Val)};
encode_read_object_resp(flag, Val) ->
    #'ApbReadObjectResp'{flag =
			     #'ApbGetFlagResp'{value = Val}};
encode_read_object_resp(generic, Val) ->
    #'ApbReadObjectResp'{generic =
			     #'ApbGetGenericResp'{value = Val}}.

decode_read_object_resp(#'ApbReadObjectResp'{counter =
						 #'ApbGetCounterResp'{value =
									  Val}}) ->
    {counter, Val};
decode_read_object_resp(#'ApbReadObjectResp'{set =
						 #'ApbGetSetResp'{value =
								      Val}}) ->
    {set, Val};
decode_read_object_resp(#'ApbReadObjectResp'{reg =
						 #'ApbGetRegResp'{value =
								      Val}}) ->
    {reg, Val};
decode_read_object_resp(#'ApbReadObjectResp'{mvreg =
						 #'ApbGetMVRegResp'{values =
									Vals}}) ->
    {mvreg, Vals};
decode_read_object_resp(#'ApbReadObjectResp'{map =
						 MapResp =
						     #'ApbGetMapResp'{}}) ->
    {map, decode_map_get_resp(MapResp)};
decode_read_object_resp(#'ApbReadObjectResp'{flag =
						 #'ApbGetFlagResp'{value =
								       Val}}) ->
    {flag, Val};
decode_read_object_resp(#'ApbReadObjectResp'{generic =
						 #'ApbGetGenericResp'{value =
									  Val}}) ->
    {generic, Val}.
...
decode_set_update(Update) ->
    #'ApbSetUpdate'{optype = OpType, adds = A, rems = R} =
	Update,
    case OpType of
      'ADD' ->
	  case A of
...
decode_counter_update(Update) ->
    ...
    case I of
      ...
      I ->
	  {increment,
	   I} % negative value for I indicates decrement
    end.
...
encode_map_update({batch, {Updates, RemovedKeys}}) ->
    UpdatesEnc = [encode_map_nested_update(U)
		  || U <- Updates],
    RemovedKeysEnc = [encode_map_key(K)
		      || K <- RemovedKeys],
    #'ApbMapUpdate'{updates = UpdatesEnc,
		    removedKeys = RemovedKeysEnc}.

decode_map_update(#'ApbMapUpdate'{updates = [Update],
				  removedKeys = []}) ->
    {update, decode_map_nested_update(Update)};
decode_map_update(#'ApbMapUpdate'{updates = Updates,
				  removedKeys = []}) ->
    {update, [decode_map_nested_update(U) || U <- Updates]};
decode_map_update(#'ApbMapUpdate'{updates = [],
				  removedKeys = [Key]}) ->
...
decode_map_update(#'ApbMapUpdate'{updates = Updates,
				  removedKeys = Keys}) ->
    {batch,
     {[decode_map_nested_update(U) || U <- Updates],
      [decode_map_key(K) || K <- Keys]}}.

encode_map_nested_update({{Key, Type}, Update}) ->
    #'ApbMapNestedUpdate'{key = encode_map_key({Key, Type}),
			  update = encode_update_operation(Type, Update)}.

decode_map_nested_update(#'ApbMapNestedUpdate'{key =
						   KeyEnc,
					       update = UpdateEnc}) ->
...
encode_map_key({Key, Type}) ->
    ?ASSERT_BINARY(Key),
    #'ApbMapKey'{key = Key, type = encode_type(Type)}.
...
encode_map_get_resp(Entries) ->
    #'ApbGetMapResp'{entries =
			 [encode_map_entry(E) || E <- Entries]}.

decode_map_get_resp(#'ApbGetMapResp'{entries =
					 Entries}) ->
    [decode_map_entry(E) || E <- Entries].

encode_map_entry({{Key, Type}, Val}) ->
    Value = encode_read_object_resp(encode_crdt_type(Type),
				    Val),
    #'ApbMapEntry'{key = encode_map_key({Key, Type}),
		   value = Value}.

decode_map_entry(#'ApbMapEntry'{key = KeyEnc,
				value = ValueEnc}) ->
    {Key, Type} = decode_map_key(KeyEnc),
...
encode_node(N) ->
    if is_atom(N) -> atom_to_binary(N, utf8);
...
encode_create_dc(Nodes) ->
    #'ApbCreateDC'{nodes = [encode_node(N) || N <- Nodes]}.

encode_create_dc_response({error, Reason}) ->
    #'ApbCreateDCResp'{success = false,
		       errorcode = encode_error_code(Reason)};
encode_create_dc_response(ok) ->
    #'ApbCreateDCResp'{success = true}.

encode_get_connection_descriptor() ->
    #'ApbGetConnectionDescriptor'{}.

encode_get_connection_descriptor_response({error,
					   Reason}) ->
    #'ApbGetConnectionDescriptorResp'{success = false,
				      errorcode = encode_error_code(Reason)};
encode_get_connection_descriptor_response({ok,
					   Descriptor}) ->
    #'ApbGetConnectionDescriptorResp'{success = true,
				      descriptor = Descriptor}.

encode_connect_to_dcs(Descriptors) ->
    #'ApbConnectToDCs'{descriptors = Descriptors}.

encode_connect_to_dcs_response({error, Reason}) ->
    #'ApbConnectToDCsResp'{success = false,
			   errorcode = encode_error_code(Reason)};
encode_connect_to_dcs_response(ok) ->
    #'ApbConnectToDCsResp'{success = true}.
...
check_response(Input) -> check_response(Input, Input).
...
check_request(Input) -> check_request(Input, Input).

%% Tests encode and decode
start_test() ->
    check_request({start_transaction, <<"opaque_binary">>,
		   []}),
    check_response({start_transaction_response,
		    {ok, <<"opaque_binary">>}}).
...
commit_test() ->
    check_request({commit_transaction,
		   <<"opaque_binary">>}),
    check_response({commit_transaction_response,
		    {ok, <<"opaque_binary">>}}).
...
read_test() ->
    Objects = [{<<"key1">>, antidote_crdt_counter_pn,
		<<"bucket1">>},
	       {<<"key2">>, antidote_crdt_set_aw, <<"bucket2">>}],
    check_request({read_objects, Objects,
		   <<"opaque_binary">>}),
    check_request({static_read_objects, <<"opaque_binary">>,
		   [], Objects}),
    Map = [{<<"key1">>, antidote_crdt_map_rr,
	    <<"bucket1">>}],
    check_request({read_objects, Map, <<"opaque_binary">>}),
    InMap = [{{<<"key1">>, antidote_crdt_map_rr,
	       <<"bucket1">>},
    ...
    In = [{{<<"key1">>, antidote_crdt_counter_pn,
	    <<"bucket1">>},
	   1},
	  {{<<"key2">>, antidote_crdt_set_aw, <<"bucket2">>},
	   [<<"a">>, <<"b">>]},
	  {{<<"key3">>, antidote_crdt_flag_dw, <<"bucket3">>},
	   true},
	  {{<<"key4">>, antidote_crdt_register_lww,
	    <<"bucket4">>},
	   <<"c">>},
	  {{<<"key4">>, antidote_crdt_register_mv, <<"bucket4">>},
	   [<<"d">>, <<"e">>]}],
    Out = [{counter, 1}, {set, [<<"a">>, <<"b">>]},
	   {flag, true}, {reg, <<"c">>},
	   {mvreg, [<<"d">>, <<"e">>]}],
    Input = {read_objects_response, {ok, In}},
    Output = {read_objects_response, {ok, Out}},
    check_response(Input, Output),
    InputStatic = {static_read_objects_response,
		   {In, <<"opaque_binary">>}},
    OutputStatic = {static_read_objects_response,
		    {Out, <<"opaque_binary">>}},
    check_response(InputStatic, OutputStatic).

update_test() ->
    Updates = [{{<<"K1">>, antidote_crdt_counter_pn,
		 <<"B">>},
		increment, 1},
	       {{<<"K2">>, antidote_crdt_counter_fat, <<"B">>},
		increment, 1},
	       {{<<"K3">>, antidote_crdt_set_aw, <<"B">>}, add,
		<<"3">>},
	       {{<<"K4">>, antidote_crdt_set_aw, <<"B">>}, add_all,
		[<<"5">>, <<"6">>]},
	       {{<<"K5">>, antidote_crdt_set_rw, <<"B">>}, remove,
		<<"3">>},
	       {{<<"K5">>, antidote_crdt_set_rw, <<"B">>}, remove_all,
		[<<"1">>, <<"2">>]},
	       {{<<"K6">>, antidote_crdt_flag_dw, <<"B">>}, enable,
		{}},
	       {{<<"K7">>, antidote_crdt_flag_ew, <<"B">>}, disable,
		{}},
	       {{<<"K8">>, antidote_crdt_register_lww, <<"B">>},
		assign, <<"Hello">>},
	       {{<<"K9">>, antidote_crdt_register_mv, <<"B">>}, assign,
		<<"World">>},
	       {{<<"K6">>, antidote_crdt_flag_ew, <<"B">>}, disable,
		{}}],
    check_request({update_objects, Updates,
		   <<"opaque_binary">>}),
    check_request({static_update_objects,
		   <<"opaque_binary">>, [], Updates}),
    check_request({update_objects,
		   [{{<<"K1">>, antidote_crdt_counter_pn, <<"B">>},
		     decrement, 10}],
		   <<"opaque_binary">>},
		  {update_objects,
		   [{{<<"K1">>, antidote_crdt_counter_pn, <<"B">>},
    ...
    check_request({update_objects,
		  ...
		  {update_objects,
		   ...
		   <<"opaque_binary">>}),
    check_request({update_objects,
		   [{{<<"K1">>, antidote_crdt_counter_pn, <<"B">>}, reset,
		     {}}],
		   <<"opaque_binary">>},
		  {update_objects,
		   [{{<<"K1">>, antidote_crdt_counter_pn, <<"B">>}, reset,
		     {}}],
		   <<"opaque_binary">>}),
    MapUpdates = [{{<<"M">>, antidote_crdt_map_go, <<"B">>},
		   update,
		   [{{<<"A">>, antidote_crdt_counter_pn}, {increment, 5}},
		    {{<<"B">>, antidote_crdt_counter_pn}, {increment, 5}}]},
		  {{<<"M">>, antidote_crdt_map_rr, <<"B">>}, remove,
		   [{<<"A">>, antidote_crdt_counter_pn},
		    {<<"B">>, antidote_crdt_set_aw}]}],
    check_request({update_objects, MapUpdates,
		   <<"opaque_binary">>}),
...
error_messages_test() ->
    ...
    check_response({read_objects_response,
		    {error, unknown}}),
    check_response({get_connection_descriptor_response,
		    {error, unknown}}),
    check_response({create_dc_response, {error, unknown}}),
    check_response({connect_to_dcs_response,
		    {error, unknown}}),
    check_response({error_response,
		    {unknown, <<"Message">>}}),
    check_response({error_response,
		    {timeout, <<"Message">>}}),
    check_response({error_response,
    ...
    check_response({error_response,
		    {aborted, <<"Message">>}}),
    check_response({error_response,
		    {{error_code, 123}, <<"Message">>}},
		   {error_response, {{error_code, 123}, <<"Message">>}}).
...
dc_management_test() ->
    ...
    check_response({get_connection_descriptor_response,
		    {ok, Descriptor}}),
    Descriptors = [<<"opaque_binary_descriptor1">>,
		   <<"opaque_binary_descriptor2">>,
		   <<"opaque_binary_descriptor3">>],
    check_request({connect_to_dcs, Descriptors}),
    check_response({connect_to_dcs_response, ok}),
    ok.
